        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            else if(g== null && n!=null)
            {
                query = "SELECT movies.id, movies.title, movies.year, movies.director, r.rating, "
                        ...
                        + " GROUP_CONCAT(DISTINCT s.name ORDER BY (SELECT COUNT(*) FROM stars_in_movies z WHERE z.starId = s.id GROUP BY s.id ) DESC, s.name) as name,"
                        + " GROUP_CONCAT(DISTINCT s.id ORDER BY (SELECT COUNT(*) FROM stars_in_movies z WHERE z.starId = s.id GROUP BY s.id ) DESC, s.name) as nameId "
                        + " FROM movies "
